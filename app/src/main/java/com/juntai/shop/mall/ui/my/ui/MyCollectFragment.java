    ...
    protected void initView() {
        ...
        adapter.setOnItemClickListener((adapter1, view, position) -> {
            ...
            if (type == 0){
                ...
                App.app.activityTool.toGoodsActivity(mContext,adapter.getData().get(position).getShopId(),adapter.getData().get(position).getCommodityId());
            }
        });
        //
    ...
    public void getData(){
        if (function == 1){
            ...
            observable = AppNetModule.createrRetrofit()
                    .collectsMy(App.app.getAccount(),
                            App.app.getUserToken(),
                            App.app.getUid(),type,
                            String.valueOf(App.app.getBdLocation().getLatitude()),
                            String.valueOf(App.app.getBdLocation().getLongitude()));
        }else{
            //我的分享
            observable = AppNetModule.createrRetrofit()
                    .shareMy(App.app.getAccount(),
                            App.app.getUserToken(),
                            App.app.getUid(),type,
                            String.valueOf(App.app.getBdLocation().getLatitude()),
                            String.valueOf(App.app.getBdLocation().getLongitude()));
    ...
    public void del(){
        if (function == 1){
            ...
            observableDel = AppNetModule.createrRetrofit()
                    .collectsOperate(App.app.getAccount(), App.app.getUserToken(),type,1,ids);
        }else{
            //我的分享
            observableDel = AppNetModule.createrRetrofit()
                    .deleteShare(App.app.getAccount(), App.app.getUserToken(),ids);
        }
